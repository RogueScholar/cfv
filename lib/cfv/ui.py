...
LISTNOTFOUND = 2048
LISTUNVERIFIED = 4096
LISTARGS = {'ok': LISTOK, 'bad': LISTBAD,
            'notfound': LISTNOTFOUND, 'unverified': LISTUNVERIFIED}
...
class View(object):
    ...
    def setup_output(self):
        ...
        progressfd = self.stdinfo.isatty(
        ) and self.stdinfo or self.stderr.isatty() and self.stderr or self.stdinfo
    ...
    def plistf(self, filename):
        self.stdout.write(self.perhaps_showpath(
            filename) + self.config.listsep)

    def ev_test_cf_begin(self, cftypename, filename, comment):
        if comment:
            comment = ', ' + comment
            # limit the length in case its a really long one.
    ...
    def ev_f_verifyerror_dupe(self, filename, msg, dupefilename, foundok):
        self.ev_f_verifyerror(filename, msg + ' (dupe of %s removed)' %
                              strutil.showfn(dupefilename), foundok)

    def ev_f_verifyerror_renamed(self, filename, msg, newfilename, foundok):
        self.ev_f_verifyerror(filename, msg + ' (renamed to %s)' %
                              strutil.showfn(newfilename), foundok)
    ...
    def ev_f_found(self, filename, filesize, filecrc, found_fn, action='found'):
        self.ev_f_ok(filename, filesize, filecrc, 'OK(%s %s)' %
                     (action, strutil.showfn(found_fn)))

    def ev_f_ok(self, filename, filesize, filecrc, msg):
        if self.config.list & LISTOK:
            self.plistf(filename)
        if filesize >= 0:
            self.pverbose('%s : %s (%i,%s)' % (
