...
def cfv_stdin_test(cmd, file):
    ...
    try:
        ...
        x = re.search(
            r"^([^\r\n]*)" + re.escape(file) +
            r"(.*)$[\r\n]{0,2}^-: (\d+) files, (\d+) OK.  [\d.]+ seconds, [\d.]+K(/s)?$",
            o1,
            re.M | re.DOTALL,
        )
        if not x:
            raise cst_err(4)
        x2 = re.search(
            r"^" + re.escape(x.group(1)) + r"[\t ]*" + re.escape(x.group(2)) +
            r"$[\r\n]{0,2}^-: (\d+) files, (\d+) OK.  [\d.]+ seconds, [\d.]+K(/s)?$",
            o2,
            re.M,
        )
        ...
        r = er
    test_log_results("stdin/out of " + cmd + " with file " + file, (s1, s2),
                     o1 + "\n" + o2, r, None)
...
def cfv_stdin_progress_test(t, file):
    ...
    try:
        try:
            ...
            s1, o1 = cfvtest.runcfv("%s --progress=yes -C -t %s -f %s %s" %
                                    (cfvcmd, t, cf1, file))
            if s1:
                raise cst_err(2)
            s2, o2 = cfvtest.runcfv("%s --progress=yes -C -t %s -f %s -" %
                                    (cfvcmd, t, cf2),
                                    stdin=file)
            if s2:
                raise cst_err(3)
            if (
                    t != "csv2"
            ):  # csv2 has only filesize, hence checksum never happens, so no progress
                x = re.match(
                    re.escape(file) + r" : (\.{20}[-\b.#\\|/]*)[ \r\n]+" +
                    "\x1b\\[K" + re.escape(cf1) +
                    r": (\d+) files, (\d+) OK.  [\d.]+ seconds, [\d.]+K(/s)?$",
                    o1,
                    re.M | re.DOTALL,
                )
                if not x:
            ...
            r = er
        test_log_results(
            "progress=yes stdin/out of " + t + " with file " + file,
            (s1, s2),
            o1 + "\n" + o2 + "\n--\n" + c1 + "\n" + c2,
            r,
            None,
...
rx_End = (
    r"(, \d+ differing cases)?(, \d+ quoted filenames)?.  [\d.]+ seconds, [\d.]+K(/s)?$"
)
...
def cfv_test(s, o, op=operator.gt, opval=0):
    ...
    if s == 0 and x and x.group(1) == x.group(2) and op(
            int(x.group(1)), opval):
        return 0
    return 1


def cfv_substatus_test(s,
                       o,
                       unv=0,
                       notfound=0,
                       badcrc=0,
    ...
    if x:
        if files == -2:
            files = reduce(operator.add,
                           [ok, badcrc, badsize, notfound, ferror])
        expected = [
            files,
            ok,
            badcrc,
...
def cfv_listdata_abs_test(s, o):
    if s == 0 and re.search(
            "^" + re.escape("\0".join(
                map(joincurpath, ["data1", "data2", "data3", "data4"]))) +
            "\0$",
            o,
            re.I,
    ):
...
def cfv_listdata_bad_test(s, o):
    if (s & 6 and not s & ~6 and re.search(
            "^(d2.)?test4.foo\0test.ext.end\0test2.foo\0test3\0$", o, re.I)):
        return 0
    return 1
...
def cfv_version_test(s, o):
    ...
    x3 = re.search(
        r" v([\d.]+(?:\.dev\d+)?):",
        open(os.path.join(cfvtest.testpath, os.pardir,
                          "Changelog")).readline(),
    )
    if x:
        log("cfv: " + x.group(1))
...
def T_test(f, extra=None):
    ...
    test_generic(cmd + " -T --list0=ok -f test" + f,
                 ...
                 stderr="/dev/null")
    test_generic(
        cmd + " -T --showpaths=n-r --list0=ok -f test" + f,
        cfv_listdata_test,
        stderr="/dev/null",
    )
    test_generic(
        cmd + " -T --showpaths=n-a --list0=ok -f test" + f,
        cfv_listdata_test,
        stderr="/dev/null",
    )
    test_generic(
        cmd + " -T --showpaths=a-a --list0=ok -f test" + f,
        cfv_listdata_test,
        stderr="/dev/null",
    )
    test_generic(
        cmd + " -T --showpaths=2-a --list0=ok -f test" + f,
        cfv_listdata_test,
        stderr="/dev/null",
    )
    test_generic(
        cmd + " -T --showpaths=y-r --list0=ok -f test" + f,
        cfv_listdata_test,
        stderr="/dev/null",
    )
    test_generic(
        cmd + " -T --showpaths=y-a --list0=ok -f test" + f,
        cfv_listdata_abs_test,
        stderr="/dev/null",
    )
    test_generic(
        cmd + " -T --showpaths=1-a --list0=ok -f test" + f,
        cfv_listdata_abs_test,
        stderr="/dev/null",
    )
    ...
    test_generic(cmd + " -v -T --list0=ok -f test" + f,
                 ...
                 stderr="/dev/null")
    test_generic(
        cmd + " -v -T --list0=unverified -f test" + f +
        " unchecked.dat testfix.csv data1",
        cfv_listdata_unv_test,
        stderr="/dev/null",
    )
...
def gzC_test(f, extra=None, verify=None, t=None, d=None):
    ...
    try:
        ...
        if extra:
            cmd += " " + extra
        test_generic("%s -q -C -t %s -zz -f - %s" % (cmd, t, d),
                     status_test,
                     stdout=f2)
    ...
    def C_test_encoding(enc):
        ...
        try:
            ...
            with open(os.path.join(d, "foo bar.baz"), "w") as f2:
                f2.write("baz")
            test_generic(
                cfvcmd + " --encoding=%s -v -C -p %s -t %s" % (enc, d, t),
                rcurry(cfv_all_test, ok=3),
            )
            test_generic(
                cfvcmd + " --encoding=%s -v -T -p %s" % (
                    enc,
                    d,
                ),
                rcurry(cfv_all_test, ok=3),
...
def C_funkynames_test(t):
    ...
    for deep in (0, 1):
        ...
        try:
            ...
            test_generic(
                cfvcmd + "%s -v -C -p %s -t %s -f %s" %
                (deep and " -rr" or "", d, t, cfn),
                rcurry(cfv_all_test,
                       files=num,
                       ok=numencodable,
                       ferror=numunencodable),
            )
            test_generic(
                cfvcmd + " -v -T -p %s -f %s" % (d, cfn),
                rcurry(cfv_all_test, files=numencodable, ok=numencodable),
            )
            test_generic(
                cfvcmd + " -v -u -T -p %s -f %s" % (d, cfn),
                rcurry(
                    cfv_all_test,
                    files=numencodable,
                    ok=numencodable,
            ...
            cfn = os.path.join(d, "funky%s.%s" % (deep and "deep" or "", t))
            test_generic(
                cfvcmd + "%s --encoding=utf-8 -v -C -p %s -t %s -f %s" %
                (deep and " -rr" or "", d, t, cfn),
                rcurry(cfv_all_test, files=num, ok=num),
            )
            test_generic(
                cfvcmd + " -v --encoding=utf-8 -T -p %s -f %s" % (d, cfn),
                rcurry(cfv_all_test, files=num, ok=num),
            )
            test_generic(
                cfvcmd + " -v --encoding=utf-8 -u -T -p %s -f %s" % (d, cfn),
        ...
        try:
            ...
            test_generic(
                cfvcmd +
                "%s --encoding=raw -v --piece_size_pow2=1 -C -p %s -t %s -f %s"
                % (deep and " -rr" or "", d3, t, dcfn),
            ...
            test_generic(
                cfvcmd + " --encoding=raw -v -T -p %s -f %s" % (d3, dcfn),
                rcurry(cfv_all_test, files=cnum, ok=cnum),
            )
            test_generic(
                cfvcmd + " --encoding=raw -v -u -T -p %s -f %s" % (d3, dcfn),
            ...
            test_generic(
                cfvcmd + " -v -T -p %s -f %s" % (d3, dcfn),
                rcurry(cfv_substatus_test, cferror=1),
            )
            # rcurry(cfv_all_test,ok=okcnum,cferror=numundecodable,unv=numundecodable))
            test_generic(
                cfvcmd + " -v -u -T -p %s -f %s" % (d3, dcfn),
                rcurry(cfv_substatus_test, cferror=1, unv=1),
            )
            # rcurry(cfv_all_test,ok=okcnum,cferror=numundecodable))
            test_generic(
                cfvcmd + " -v -m -T -p %s -f %s" % (d3, dcfn),
                rcurry(cfv_substatus_test, cferror=1),
            )
            # rcurry(cfv_all_test,ok=okcnum,cferror=numundecodable,unv=numundecodable))
            test_generic(
                cfvcmd + " -v -m -u -T -p %s -f %s" % (d3, dcfn),
                rcurry(cfv_substatus_test, cferror=1, unv=1),
            )
            ...
            if not deep:
                ...
                if t != "torrent":
                    test_generic(
                        cfvcmd + " --encoding=raw -v -s -T -p %s -f %s" %
                        (d3, dcfn),
                        rcurry(cfv_all_test, ok=cnum, misnamed=numrenamed),
                    )
                if fmt_hassize(t):
                    test_generic(
                        cfvcmd + " --encoding=raw -v -m -s -T -p %s -f %s" %
                        (d3, dcfn),
                        rcurry(cfv_all_test, ok=cnum, misnamed=numrenamed),
                    )
                ...
                dcfn = os.path.join(
                    d3, "funky3%s2.%s" % (deep and "deep" or "", t))
                test_generic(
                    cfvcmd + "%s -v -C -p %s -t %s -f %s" %
                    (deep and " -rr" or "", d3, t, dcfn),
        ...
        try:
            ...
            numerr = len(ulist) - okcnum
            dcfn = os.path.join(d3,
                                "funky3%s3.%s" % (deep and "deep" or "", t))
            ...
            test_generic(
                cfvcmd + " -v -u -T -p %s -f %s" % (d3, dcfn),
                rcurry(cfv_all_test, ok=okcnum, unv=numerr),
            )
...
def ren_test(f, extra=None, verify=None, t=None):
    ...
    try:
        ...
        os.mkdir(dir2)
        fls = [
            join(dir, "test.ext.end"),
            join(dir, "test2.foo"),
            join(dir, "test3"),
            join(dir2, "test4.foo"),
        ]
        flsf = [
            join(dir, "test.ext-%i.end"),
            join(dir, "test2-%i.foo"),
            join(dir, "test3-%i"),
            join(dir2, "test4-%i.foo"),
        ]
        flsf_1 = [
            join(dir, "test.ext.end-%i"),
        ...
        flsw(b"1")
        test_generic(
            basecmd + " --showpaths=0 -v -T --list0=bad",
...
def search_test(t, test_nocrc=0, extra=None):
    ...
    if not hascrc and not hassize:
        ...
        try:
            ...
            test_generic(cmd + " -v -T -p %s -f %s" % (d, cfn),
                         rcurry(cfv_all_test, notfound=4))
            test_generic(
                cmd + " -v -s -T -p %s -f %s" % (d, cfn),
                rcurry(cfv_all_test, notfound=4),
            )
            test_generic(
                cmd + " -v -s -n -T -p %s -f %s" % (d, cfn),
                rcurry(cfv_all_test, notfound=4),
            )
            test_generic(
                cmd + " -v -s -u -T -p %s -f %s" % (d, cfn),
                rcurry(cfv_all_test, notfound=4, unv=4),
            )
        finally:
            shutil.rmtree(d)
    ...
    try:
        ...
        test_generic(cmd + " -v -T -p %s -f %s" % (d, cfn),
                     rcurry(cfv_all_test, notfound=4))
        test_generic(cmd + " -v -s -T -p %s -f %s" % (d, cfn),
                     dont_find_same_file_twice_test)
        test_generic(cmd + " -v -T -p %s -f %s" % (d, cfn),
                     rcurry(cfv_all_test, notfound=4))
        test_generic(
            cmd + " -v -n -s -T -p %s -f %s" % (d, cfn),
            rcurry(cfv_all_test, ok=4, misnamed=4),
        )
        test_generic(cmd + " -v -u -T -p %s -f %s" % (d, cfn),
    ...
    try:
        ...
        test_generic(
            cmd + " -v -T -p %s -f %s" % (d, cfn),
            rcurry(cfv_all_test, notfound=1, **experrs),
        )
        test_generic(
            cmd + " -v -s -T -p %s -f %s" % (d, cfn),
            rcurry(cfv_all_test, ok=4, misnamed=misnamed1),
        )
        test_generic(
            cmd + " -v -T -p %s -f %s" % (d, cfn),
            rcurry(cfv_all_test, notfound=1, **experrs),
        ...
        test_generic(
            cmd + " -v -n -s -T -p %s -f %s" % (d, cfn),
            rcurry(cfv_all_test, ok=4, misnamed=misnamed2),
        )
        test_generic(cmd + " -v -u -T -p %s -f %s" % (d, cfn),
                     rcurry(cfv_all_test, ok=4))
    finally:
    ...
    if hasattr(os, "symlink"):
        ...
        try:
            ...
            test_generic(
                cmd + " -v -T -p %s -f %s" % (d, cfn),
                rcurry(cfv_all_test, ok=1, notfound=3),
            )
        finally:
            shutil.rmtree(d)
    ...
    if fmt_cancreate(t):
        ...
        try:
            ...
            shutil.copyfile("data1",
                            os.path.join(d, "aOeU.AoEu", "boO.FaRr", "DaTa1"))
            test_generic(
                cmd + " -v -rr -C -p %s -t %s -f %s" % (d, t, dcfn),
                rcurry(cfv_all_test, files=1, ok=1),
            )
            os.rename(
                os.path.join(d, "aOeU.AoEu", "boO.FaRr", "DaTa1"),
                os.path.join(d, "aOeU.AoEu", "boO.FaRr", "Foo1"),
            )
            shutil.copyfile("data4",
                            os.path.join(d, "aOeU.AoEu", "boO.FaRr", "DaTa1"))
            test_generic(
                cmd + " -v -s -T -p %s -f %s" % (d, dcfn),
            ...
            if hassize:
                ...
                experrs = {"badcrc": 1}
            test_generic(
                cmd + " -i -v -T -p %s -f %s" % (d, dcfn),
                rcurry(cfv_all_test, files=1, ok=0, **experrs),
            )
            test_generic(
                cmd + " -i -v -s -n -T -p %s -f %s" % (d, dcfn),
                rcurry(cfv_all_test, files=1, ok=1, misnamed=1),
            )
            test_generic(
                cmd + " -i -v -T -p %s -f %s" % (d, dcfn),
...
"aa4/bb4/data4" FA323C6D""")
        ...
        test_generic(
            cfvcmd +
            r" -v --unquote=yes --strippaths=0 --fixpaths \\/ -s -T -p " + d,
...
def deep_unverified_test():
    ...
    try:
        ...
        datafns = (
            ...
            join(B, "daTA3"),
            join(B, "uNv3"),
            join(B_ushallow, "uNvs"),
            join(B_ushallow_d, "unvP"),
            join(B_ushallow_d, "datA5"),
            join(u, "uNVu"),
            join(u, "UnvY"),
            join(u_u2, "UNVX"),
            join(e2_e2s, "DaTaE"),
        ...
        test_generic(
            cfvcmd + " -i -U -p " + dir + " " + " ".join(lower_datafns),
            r_test)
        test_generic(
            cfvcmd + " -i -u -p " + dir + " " + " ".join(lower_datafns),
            r_unv_verbose_test,
        ...
        test_generic(
            cfvcmd + " -i -uu -p " + dir + " " + " ".join(lower_datafns),
            r_unv_verbose_test,
        )
    finally:
        shutil.rmtree(dir)
    ...
    try:
        ...
        for t in allcreatablefmts():
            if fmt_istext(t):
                ...
                test_generic(
                    cfvcmd + " -C --encoding=utf-8 -p %s -t %s -f %s" %
                    (datad, t, utf8cfn),
                ...
                for enc in utfencodings:
                    ...
                    try:
                        ...
                        test_generic(
                            cfvcmd + " -T -p %s -t %s -f %s" %
                            (datad, t, bommedcfn),
                            rcurry(cfv_all_test, ok=fnok),
                        )
                        test_generic(
                            cfvcmd + " -T -p %s -f %s" % (datad, bommedcfn),
                            rcurry(cfv_all_test, ok=fnok),
                        )
    finally:
        shutil.rmtree(d)
...
def test_encoding2():
    ...
    try:
        ...
        dirn = list(filter(lambda s: not s.endswith("torrent"),
                           os.listdir(d)))[0]
        try:
            files = [
                os.path.join(dirn, s)
...
def largefile2GB_test():
    ...
    try:
        ...
        f.write("bar")
        f.close()
        test_generic(cfvcmd + " -v -T -p %s" % "bigfile2",
                     rcurry(cfv_all_test, ok=6))
    finally:
        os.unlink(fn)
...
def largefile4GB_test():
    ...
    try:
        ...
        f.write("baz")
        f.close()
        test_generic(cfvcmd + " -v -T -p %s" % "bigfile",
                     rcurry(cfv_all_test, ok=10))
    finally:
        os.unlink(fn)
...
def manyfiles_test(t):
    ...
    try:
        ...
        test_generic(cfvcmd + " -C -p %s -t %s -f %s" % (d, t, cfn),
                     rcurry(cfv_all_test, ok=num))
        test_generic(cfvcmd + " -T -p %s -f %s" % (d, cfn),
                     rcurry(cfv_all_test, ok=num))
    finally:
        shutil.rmtree(d)
...
def specialfile_test(cfpath):
    ...
    try:
        ...
        t.start()
        s, o = cfvtest.runcfv("%s --progress=yes -T -p %s -f %s" %
                              (cfvcmd, d, cfn))
        t.join()
        r = 0
        if s:
...
def private_torrent_test():
    ...
    try:
        ...
        data = readfile(f)
        test_log_results("should not contain private flag", 0, repr(data),
                         needle in data, None)

        f = os.path.join(tmpd, "test2.torrent")
        test_generic("%s --private_torrent -C -f %s data1" % (cmd, f),
                     cfv_test)
        data = readfile(f)
        test_log_results("should contain private flag", 0, repr(data),
                         needle not in data, None)
    finally:
        shutil.rmtree(tmpd)
...
def all_unittest_tests():
    ...
    if not result.wasSuccessful():
        ...
        r = 0
    test_log_finish("all_unittests_suite", not result.wasSuccessful(), r, None,
                    None)
...
try:
    optlist, args = getopt.getopt(sys.argv[1:], "ie",
                                  ["long", "help", "unit", "exit-early"])
...
logfile = open(
    os.path.join(
        tempfile.gettempdir(),
        "cfv_%s_test-%s.log" %
        (cfvtest.ver_cfv, time.strftime("%Y%m%dT%H%M%S")),
    ),
    "w",
)
...
def all_tests():
    ...
    test_generic(
        cfvcmd + " -T -v -f testencodingcomment.torrent",
    ...
    test_generic(cfvcmd + " -ri -p a " + os.path.join("c", "fOo.BaR"),
                 cfv_test)
    test_generic(cfvcmd + " -r -u -p a " + os.path.join("C", "foo.bar"),
                 cfv_test)
    test_generic(cfvcmd + " -ri -u -p a " + os.path.join("c", "fOo.BaR"),
                 cfv_test)
    ...
    test_generic(cfvcmd + " --strippaths=none -T -f teststrip-none.csv4",
                 cfv_notfound_test)
    test_generic(
        cfvcmd + r" --strippaths=0 --fixpaths \\/ -T -f testdrivestrip.md5",
        rcurry(cfv_all_test, ok=4),
    )
    test_generic(
        cfvcmd +
        r" --strippaths=0 --unquote=yes --fixpaths \\/ -T -f testdrivestripquoted.md5",
        rcurry(cfv_all_test, ok=4),
    )
    test_generic(
        cfvcmd +
        r" --strippaths=0 --unquote=yes --fixpaths \\/ -T -f testdrivestripquoted.md5 data1 data3 data4",
        rcurry(cfv_all_test, ok=3),
    )

    test_generic(cfvcmd + " -i -T -f testcase.csv", cfv_test)
    test_generic(cfvcmd + " -T --unquote=yes -f testquoted.sfv", cfv_test)
    test_generic(cfvcmd + " -i --unquote=yes -T -f testquotedcase.sfv",
                 cfv_test)
    test_generic(
        cfvcmd + " -i --unquote=yes -T -f testquotedcase.sfv DaTa1 " +
        os.path.join("a", "C", "Foo.bar"),
    ...
    if pathfind(
            "cksfv"
    ):  # don't report pointless errors on systems that don't have cksfv
    ...
    for t in allavailablefmts():
        if fmt_istext(t):
            test_generic(
                cfvcmd + " --encoding=cp500 -T -f test." + t,
                rcurry(cfv_all_test, cferror=1),
            )
        else:
            if t == "par":
                try:
                    ...
                    err = 0
                test_generic(
                    cfvcmd + " --encoding=utf-16be -T -f test." + t,
                    rcurry(cfv_all_test, notfound=nf, ferror=err),
                )
                test_generic(
                    cfvcmd + " --encoding=cp500 -T -f test." + t,
                    rcurry(cfv_all_test, cferror=4),
                )
                test_generic(
                    cfvcmd + " --encoding=cp500 -i -T -f test." + t,
                    rcurry(cfv_all_test, cferror=4),
                )
            else:
                try:
    ...
    test_generic(
        cfvcmd +
        " -m -v -T -t sfv", lambda s, o: cfv_typerestrict_test(s, o, "sfv"))
    test_generic(
        cfvcmd + " -m -v -T -t sfvmd5",
        lambda s, o: cfv_typerestrict_test(s, o, "sfvmd5"),
    )
    test_generic(
        cfvcmd + " -m -v -T -t bsdmd5",
        lambda s, o: cfv_typerestrict_test(s, o, "bsdmd5"),
    )
    for fmt in coreutilsfmts():
        test_generic(
            cfvcmd + " -m -v -T -t " + fmt,
            lambda s, o: cfv_typerestrict_test(s, o, fmt),
        )
    test_generic(
        cfvcmd +
        " -m -v -T -t csv", lambda s, o: cfv_typerestrict_test(s, o, "csv"))
    test_generic(
        cfvcmd +
        " -m -v -T -t par", lambda s, o: cfv_typerestrict_test(s, o, "par"))
    test_generic(
        cfvcmd +
        " -m -v -T -t par2", lambda s, o: cfv_typerestrict_test(s, o, "par2"))

    test_generic(
        cfvcmd + " -u -t md5 -f test.md5 data* unchecked.dat test.md5",
        cfv_unv_test)
    test_generic(cfvcmd + " -u -f test.md5 data* unchecked.dat", cfv_unv_test)
    test_generic(cfvcmd + " -u -f test.md5 data* unchecked.dat test.md5",
                 cfv_unv_test)
    test_generic(
        cfvcmd +
        r" -i -tcsv --fixpaths \\/ -Tu", lambda s, o: cfv_unv_test(s, o, None))
    test_generic(cfvcmd + " -T -t md5 -f non_existant_file", cfv_cferror_test)
    test_generic(
        cfvcmd + " -T -f " + os.path.join("corrupt", "missingfiledesc.par2"),
        cfv_cferror_test,
    )
    test_generic(
        cfvcmd + " -T -f " + os.path.join("corrupt", "missingmain.par2"),
        cfv_cferror_test,
    )
    test_generic(
        cfvcmd + " -T -m -f " +
        os.path.join("corrupt", "missingfiledesc.par2"),
        cfv_cferror_test,
    )
    test_generic(
        cfvcmd + " -T -m -f " + os.path.join("corrupt", "missingmain.par2"),
        cfv_cferror_test,
    )

    test_generic(cfvcmd + " -T -f foo.torrent", cfv_test)
    test_generic(
        cfvcmd + " -T --strip=none -p foo -f ../foo.torrent",
        rcurry(cfv_all_test, notfound=7),
    )
    for strip in (0, 1):
        test_generic(
            cfvcmd + " -T --strippaths=%s -p foo -f %s" %
            (strip, os.path.join(os.pardir, "foo.torrent")),
            rcurry(cfv_all_test, ok=7),
        )
        test_generic(
            cfvcmd + " -T --strippaths=%s -p foo2err -f %s" %
            (strip, os.path.join(os.pardir, "foo.torrent")),
            rcurry(cfv_all_test, ok=4, badcrc=3),
        )
        test_generic(
            cfvcmd + " -T --strippaths=%s -p foo2err -f %s foo1 foo4" %
            (strip, os.path.join(os.pardir, "foo.torrent")),
            rcurry(cfv_all_test, ok=0, badcrc=2),
        )
        test_generic(
            cfvcmd + " -T --strippaths=%s -p foo2err1 -f %s" %
            (strip, os.path.join(os.pardir, "foo.torrent")),
            rcurry(cfv_all_test, ok=6, badcrc=1),
        )
        test_generic(
            cfvcmd + " -T --strippaths=%s -p foo2err1 -f %s foo1 foo4" %
            (strip, os.path.join(os.pardir, "foo.torrent")),
            rcurry(cfv_all_test, ok=2),
        )
        test_generic(
            cfvcmd + " -T --strippaths=%s -p foo2badsize -f %s" %
            (strip, os.path.join(os.pardir, "foo.torrent")),
            rcurry(cfv_all_test, ok=5, badsize=1, badcrc=1),
        )
        test_generic(
            cfvcmd + " -T --strippaths=%s -p foo2badsize -f %s foo1 foo4" %
            (strip, os.path.join(os.pardir, "foo.torrent")),
            rcurry(cfv_all_test, ok=1, badcrc=1),
        )
        test_generic(
            cfvcmd + " -T --strippaths=%s -p foo2missing -f %s" %
            (strip, os.path.join(os.pardir, "foo.torrent")),
            rcurry(cfv_all_test, ok=4, badcrc=2, notfound=1),
        )
        test_generic(
            cfvcmd + " -T --strippaths=%s -p foo2missing -f %s foo1 foo4" %
            (strip, os.path.join(os.pardir, "foo.torrent")),
            rcurry(cfv_all_test, ok=0, badcrc=2),
        )
    d = tempfile.mkdtemp()
    try:
        ...
        test_generic(
            cmd + " -C -p %s -f foo.torrent" % d,
            rcurry(cfv_all_test, files=1, cferror=1),
        )
        test_log_results(
            "non-creation of empty torrent on missing announceurl?",
            "",
            repr(os.listdir(d)),
            len(os.listdir(d)) > 1,
            {},
    ...
    test_generic(cfvcmd + " -t aoeu",
                 rcurry(cfv_cftypehelp_test, 1),
                 stdout="/dev/null")
    test_generic(cfvcmd + " -t aoeu",
                 rcurry(cfv_nooutput_test, 1),
                 stderr="/dev/null")
    test_generic(cfvcmd + " -t help",
                 rcurry(cfv_cftypehelp_test, 0),
                 stderr="/dev/null")
    test_generic(cfvcmd + " -t help",
                 rcurry(cfv_nooutput_test, 0),
                 stdout="/dev/null")

    test_generic(cfvcmd + " -h", cfv_nooutput_test, stdout="/dev/null")
    test_generic(cfvcmd + " -h", cfv_version_test, stderr="/dev/null")

    donestr = "\n>>> tests finished:  ok: %i  failed: %i" % (stats.ok,
                                                             stats.failed)
    log(donestr)
    print(donestr)
